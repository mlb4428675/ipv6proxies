...
#{
  ...
  rnd_ip_block ()
  {
      ...
      echo $network:$a:$b:$c:$d >> ip.list #For a /64 network, 4 blocks are required: a, b, c, d #If the network is /48, then 5 blocks, i.e. + e block separated by a colon
  }
  ...
  for i in `cat ip.list`; do
      ...
      # If network is 64 then $i/64 if 48 then $i/48
      ifconfig venet0 inet6 add $i/64
      ifconfig venet0:0 inet6 add $i/64
  done
